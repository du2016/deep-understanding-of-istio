...
在Envoy中，引导文件包含两个ConfigSource 消息，一个指示如何获取侦听器资源，另一个指示如何获取群集资源。它还包含一个单独的ApiConfigSource消息，该消息指示如何联系ADS服务器，只要ConfigSource消息（在引导文件中或从管理服务器获取的侦听器或群集资源中）包含AggregatedConfigSource消息，就会使用该消息。
